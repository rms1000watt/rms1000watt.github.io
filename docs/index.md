# Tech Tech Tech

Useful references across Software, IT, DevOps, and Security.

## [Dev](/dev)

### [Golang](/dev/golang)

- [Articles](/dev/golang/#articles)
- [Tutorials](/dev/golang/#tutorials)
- [AwesomeGo](/dev/golang/#awesomego)

### [ReactJS](/dev/reactjs)

### [Python](/dev/python)

- [Concurrency](/dev/python/#concurrency)
- [Servers](/dev/python/#servers)

### [HTML](/dev/html)

- [Templates](/dev/html/#templates)
- [Generators](/dev/html/#generators)
- [Assets](/dev/html/#assets)
- [CSS](/dev/html/#css)

### [Serialization](/dev/serialization)

- [YAML](/dev/serialization/#yaml)
- [Protobuf](/dev/serialization/#protobuf)
- [Flatbuf](/dev/serialization/#flatbuf)
- [JSON](/dev/serialization/#json)

## [DevOps](/devops)

### [Kubernetes](/kubernetes)

### [Serverless](/serverless)

### [AWS](/aws)

- [SAM](/devops/aws/#sam)

## [Hack](/hack)

### [SBCs](/hack/sbcs)

- [RaspberryPi](/hack/sbcs/#raspberrypi)
- [CHIP](/hack/sbcs/#chip)

### [Security](/hack/security)

- [Articles](/hack/security/#articles)
- [Blogs](/hack/security/#blogs)
- [Software](/hack/security/#software)
- [Services](/hack/security/#services)

## [About](/about)

